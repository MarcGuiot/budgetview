package com.budgetview.android.shadow;
...
public class CustomShadowTabHost extends ShadowFrameLayout {
  private List<TabHost.TabSpec> tabSpecs = new ArrayList<TabHost.TabSpec>();
  private TabHost.OnTabChangeListener listener;
  private int currentTab = -1;

  public static CustomShadowTabHost create(TabHost instance) {
    return (CustomShadowTabHost)Robolectric.getShadowWrangler().shadowOf(instance);
  }
  ...
  public void setCurrentTabByTag(String tag) {
    for (int x = 0; x < tabSpecs.size(); x++) {
      TabHost.TabSpec tabSpec = tabSpecs.get(x);
  ...
  public TabHost.TabSpec getTabSpecAt(int i) {
    return tabSpecs.get(i);
  }
}
