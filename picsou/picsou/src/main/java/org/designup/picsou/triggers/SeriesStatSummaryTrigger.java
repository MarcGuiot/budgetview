...
public class SeriesStatSummaryTrigger implements ChangeSetListener {
  public void globsChanged(ChangeSet changeSet, final GlobRepository repository) {
    ...
    final Integer referenceMonth = getReferenceMonth(repository);
  ...
  public void globsReset(GlobRepository repository, Set<GlobType> changedTypes) {
    if (changedTypes.contains(Series.TYPE)) {
      Integer referenceMonth = getReferenceMonth(repository);
  ...
  private Integer getReferenceMonth(GlobRepository repository) {
    return repository.get(CurrentMonth.KEY).get(CurrentMonth.LAST_TRANSACTION_MONTH);
  }
}
