...
public abstract class AbstractLineChart extends View implements GlobSelectionListener, ChangeSetListener {
  ...
  protected void setRowColor(String row, PicsouColors topColor, PicsouColors bottomColor, final AbstractRenderer renderer, XYDataset dataset) {
    ...
    if (rowIndex >= 0) {
      colorService.addListener(new FillColorUpdater(renderer, rowIndex, topColor, bottomColor));
    }
  }

  protected void setShapeColor(String row, final PicsouColors color, final AbstractRenderer renderer, XYDataset dataset) {
    final int rowIndex = dataset.indexOf(row);
    if (rowIndex >= 0) {
      colorService.addListener(new ColorChangeListener() {
        public void colorsChanged(ColorLocator colorLocator) {
          renderer.setSeriesFillPaint(rowIndex, colorLocator.get(color.toString()));
        }
      });
    }
  }
  ...
  private static class FillColorUpdater implements ColorChangeListener {
