...
import com.budgetview.gui.categorization.components.SeriesChooserComponentFactory;
import com.budgetview.gui.description.stringifiers.SeriesNameComparator;
...
public class FilteredRepeats {
  ...
  public void update(GlobList currentTransactions) {
    for (Pair<CategorizationFilter, GlobRepeat> filter : list.values()) {
      filter.getFirst().filterForTransactions(currentTransactions);
  ...
  public boolean updateAndCheckMatch(Glob series, Glob transaction) {
    ...
    for (Pair<CategorizationFilter, GlobRepeat> filter : list.values()) {
      ...
      categorizationFilter.filterForTransactions(new GlobList(transaction));
