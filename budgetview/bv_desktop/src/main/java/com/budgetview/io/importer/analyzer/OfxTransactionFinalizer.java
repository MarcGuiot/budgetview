...
public class OfxTransactionFinalizer extends AbstractTransactionTypeFinalizer {
  public static final Pattern NAME_REGEXP = Pattern.compile("\\{NAME\\.([0-9]+)\\}");
  public static final Pattern MEMO_REGEXP = Pattern.compile("\\{MEMO\\.([0-9]+)\\}");
  public static final Pattern CHECK_NUM_REGEXP = Pattern.compile("\\{NUM\\.([0-9]+)\\}");
  public static final Pattern TYPE_REGEXP = Pattern.compile("\\{TYPE\\.([0-9]+)\\}");
  ...
  public boolean processTransaction(Glob transaction, GlobRepository repository) {
    if (Transaction.getImportType(transaction) != ImportType.OFX) {
      return false;
    }
