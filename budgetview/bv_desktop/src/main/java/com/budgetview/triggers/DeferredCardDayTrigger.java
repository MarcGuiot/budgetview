    ...
    while (startMonth <= endMonth) {
      ...
      if (globs.isEmpty()) {
        repository.create(DeferredCardDate.TYPE,
                          ...
                          value(DeferredCardDate.DAY, Month.getDay(account.get(Account.DEFERRED_DEBIT_DAY), startMonth)));
      }
      else {
      ...
      if (seriesIndex != null) {
        ...
        if (transaction != null) {
          ...
          repository.update(deferredCardDay.getKey(), DeferredCardDate.DAY,
                            Month.getDay(account.get(Account.DEFERRED_DEBIT_DAY), deferredCardDay.get(DeferredCardDate.MONTH)));
        }
      }
      else {
        repository.update(deferredCardDay.getKey(), DeferredCardDate.DAY,
