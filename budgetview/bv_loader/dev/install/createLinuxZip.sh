...
VERSION=`java -jar ../bv_desktop/target/obfuscated/budgetview.jar -v -jar | grep "Jar version"`
JAR_VERSION=`echo $VERSION | sed -e 's/Jar version://g' | sed -e 's/  *//g'`

SOFT_VERSION=`java -jar ../bv_desktop/target/obfuscated/budgetview.jar -v -soft | grep "Software version:" |
              sed -e 's/Software version://g' | sed -e 's/  *//g'`

rm -f ../bv_desktop/target/obfuscated/budgetview${JAR_VERSION}.jar
cp ../bv_desktop/target/obfuscated/budgetview.jar ../bv_desktop/target/obfuscated/budgetview${JAR_VERSION}.jar
bzip2 -c ../bv_desktop/ChangeLogOutput.txt > ../bv_desktop/ChangeLogOutput-${JAR_VERSION}.txt.bz2
mkdir -p src/test/resources/jars
cp ../bv_desktop/target/obfuscated/budgetview.jar src/test/resources/jars/budgetview.jar
...
cp ../bv_desktop/target/obfuscated/budgetview${JAR_VERSION}.jar budgetview/
